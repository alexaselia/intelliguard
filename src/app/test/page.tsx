...
const TestPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRecordings = async () => {
      ...
      // Flatten and sort recordings
      const flattenedRecordings = Object.values(data).flat();
      flattenedRecordings.sort((a, b) => a.startTime - b.startTime);
      setRecordings(flattenedRecordings);

      // Set the first recording in the playlist as the initial selected recording
      if (flattenedRecordings.length > 0) {
        const firstRecording = flattenedRecordings[0];
        const cameraIdHash = md5Hash(cameraId);
        setIsLive(false);
        setSelectedRecording(`https://nginx.megaguardiao.com.br/play/${cameraIdHash}/${firstRecording.path}`);
      }
    };
  ...
  const handleVideoEnded = () => {
    const currentIndex = recordings.findIndex(recording => recording.path === selectedRecording?.split('/').pop());
    if (currentIndex !== -1 && currentIndex + 1 < recordings.length) {
      const nextRecording = recordings[currentIndex + 1];
      const cameraIdHash = md5Hash(cameraId!);
      setSelectedRecording(`https://nginx.megaguardiao.com.br/play/${cameraIdHash}/${nextRecording.path}`);
    }
  };
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-8">
            ...
            <div className="w-full h-full md:w-1/3">
              <RecordingsAccordion
                ...
                getFilteredRecordings={() => ({})} // Not needed in this implementation
                renderDay={renderDay}
                sortOrder="oldest"
              />
            </div>
